...
\MALT, an acronym for \itt{MEGAN alignment tool} or \iit{MEGAN alignment tool}, is a sequence alignment and analysis tool designed for processing high-throughput sequencing data, especially in the context of metagenomics.
It is an extension of MEGAN, the \itt{MEGenome Analyzer} and is designed to provide the input for MEGAN,
...
\begin{itemize}
\item \pconcept{Academic license} This license permits use of the software exclusively for academic research
...
\item   \pconcept{Single user license} This license permits a single user to use the program.
This type of license is granted for a charge from the University of T\"ubingen, please contact Daniel Huson for details.
\item   \pconcept{Site license} This license permits use of the program at a single physical location, within
  a single organization.
This type of license is granted for a charge from the University of T\"ubingen, please contact Daniel Huson for details.
  \item   \pconcept{Enterprise license} This license permits use of the program anywhere within
  a single organization.
This type of license is granted for a charge from the University of T\"ubingen,please contact Daniel Huson for details.
\item \pconcept{Evaluation license} This type of license is  granted for 45 days and is for evaluation purposes only.
...
Use {\tt -s2t} for a file that maps \pconcept{synonyms} to taxon ids. A synonym is any word that may occur in
...
\begin{itemize}
\setlength{\itemindent}{30pt}
\item[\itt{--taxTree}] Use to specify the taxonomic tree, for example \itt{malt/data/ncbi.tre.gz}.
\item[\itt{--taxMap} ]Use to specify the associated map file, for example  \itt{malt/data/ncbi.map.gz}.
\item[\itt{--cogMappingFile}] Use to specify the COG mapping file, for example \itt{malt/data/cog.map.gz}.
\end{itemize}
