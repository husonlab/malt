...
public class MaltRun2 {
    ...
    public void run(String[] args) throws UsageException, IOException, CanceledException, InterruptedException {
        ...
        if (inputFileNames.size() == 1 || options.isDoHelp()) {
            ...
            if (isList & inputFileNames.size() > 0)
                inputFileNames = Basic.getAllLines(inputFileNames.get(0));
        }
        ...
        */
        ...
        malt.util.Utilities.checkFileExists((new File(indexDirectory, "mask.idx")).getPath(), true);
        ...
        switch (maltOptions.getMode()) {
            case BlastN:
                ...
                queryAlphabet = new DNA5();
                refAlphabet = new DNA5();
            ...
            case BlastX:
                ...
                queryAlphabet = new DNA5();
                refAlphabet = new ProteinAlphabet();
            ...
            case BlastP:
                ...
                queryAlphabet = new ProteinAlphabet();
                refAlphabet = new ProteinAlphabet();
        ...
        final ReferenceStore refStore = new ReferenceStore(refAlphabet, index.getNumberOfSeedShapes(), index.getNumberOfSequences());
        refStore.loadFastAFile(malt.util.Utilities.getFile(indexDirectory, "ref.idx", true));
        refStore.loadMaskFile(malt.util.Utilities.getFile(indexDirectory, "mask.idx", true));
        ...
        final SeedShape[] seedShapes = index.getSeedShapes();
        ...
        final SeedStore[][] allRefSeedStores = new SeedStore[keepRefSeeds ? seedShapes.length : 1][];
        ...
        for (String inFile : inputFileNames) {
            try {
                if ((new File(inFile).exists())) {
                    System.err.println("-----------------------------");
                    // setup input file:
                    FastAFileIteratorBytes queryIterator = new FastAFileIteratorBytes(inFile, queryAlphabet);
                    ...
                    final QueryStore queryStore = new QueryStore(queryMatchesCache, queryAlphabet, queryType, maltOptions.isDoForward(), maltOptions.isDoReverse(),
                            maltOptions.getMinComplexity(), storeOriginalQuerySequences, maltOptions.getQueryBatchSize());
                    int queryBatch = 0;
                    int numberOfQueries = 0;
                    int numberLowComplexity = 0;
                    while (queryIterator.hasNext()) { // while there are more queries to load as a batch...
                        queryBatch++;
                        ...
                        for (int s = 0; s < seedShapes.length; s++) {
                            ...
                            for (int chunk = 0; chunk < maltOptions.getNumberOfChunks(); chunk++) {
                                ...
                                final SeedStore[] refSeedStores;
                                // setup the datasetstructures:
                                {
                                    final ProgressPercentage progress = new ProgressPercentage("Setting up data structures (" + numberOfJobs + ")", numberOfJobs + 1);

                                    // setup the array of ref seed store
                                    if (keepRefSeeds) {
                                        if (allRefSeedStores[s] == null)
                                            allRefSeedStores[s] = new SeedStore[numberOfJobs];
                                        refSeedStores = allRefSeedStores[s];
                                    } else {
                                        if (allRefSeedStores[0] == null)
                                            allRefSeedStores[0] = new SeedStore[numberOfJobs];
                                        else if (allRefSeedStores[0].length < allRefSeedStores[0].length)
                                            allRefSeedStores[0] = Utilities.grow(allRefSeedStores[0], numberOfJobs);
                                        refSeedStores = allRefSeedStores[0];
                                    }
                                    progress.incrementProgress();

                                    for (int h = 0; h < numberOfJobs; h++) {
                                        if (querySeedStores[h] == null) {
                                            querySeedStores[h] = new SeedStore(seedShapes[s].getAlphabet(), seedShapes[s].getWeight(),
                                                    seedShape.getEstimatedSeedCount(queryStore.getNumberOfSequences(), queryStore.getNumberOfLetters(), maltOptions.getNumberOfChunks() * numberOfJobs));
                                        } else
                                            querySeedStores[h].reset(seedShapes[s].getAlphabet(), seedShapes[s].getWeight());
                                        if (refSeedStores[h] == null) {
                                            refSeedStores[h] = new SeedStore(seedShapes[s].getAlphabet(), seedShapes[s].getWeight(), index.getSeedCounts()[s] / (maltOptions.getNumberOfChunks() * numberOfJobs));
                                            isRefSeedsAreSet = false;
                                        } else if (!keepRefSeeds) {
                                            refSeedStores[h].reset(seedShapes[s].getAlphabet(), seedShapes[s].getWeight());
                                            isRefSeedsAreSet = false;
                                        } else if (maltOptions.getNumberOfChunks() == 1) {
                                            throw new RuntimeException("Internal error, illegal parameter combination: keepRefSeeds==true && numberOfChunks==1");
                                        }
                                        progress.incrementProgress();
                                    }
                                    progress.close();
                                } // end of setup of datastructures

                                if (!keepRefSeeds || !isRefSeedsAreSet) {
                                    ReferenceSeedsLoop.run(threadPool, chunk, maltOptions.getNumberOfChunks(), numberOfJobs, maltOptions.getNumberOfThreads(), refStore, seedShape, refSeedStores);
                                }
                                ...
                                MainAlignmentLoop.run(threadPool, maltOptions, alignerOptions, index.getMaxRefOccurrencesPerSeed(), queryStore, querySeedStores, refStore, refSeedStores, seedShape, seedShapes, matches);
                            }
                        }
    ...
    private int computeNumberOfJobs(final int numberOfChunks, final int numberOfThreads, final MaltIndexFile index, final QueryStore queryStore, final int queryBatchSize) {
        ...
        for (int t = 0; t < index.getSeedShapes().length; t++) {
            SeedShape seedShape = index.getSeedShapes()[t];
